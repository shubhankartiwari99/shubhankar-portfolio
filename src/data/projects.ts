...
export const projects: Project[] = [
  {
    ...
    shortDescription:
      "Production-grade system for measuring LLM behavioral consistency via Monte Carlo sampling. Benchmarked Qwen 2.5-7B across 310 inference calls — found temperature is not a reliability control knob.",
...
The platform runs Monte Carlo sampling (n=5 per prompt) across a temperature sweep and computes instability, confidence, semantic dispersion, and entropy for each inference. A DBSCAN clustering pass groups semantically similar outputs to identify whether a model converges or diverges on a given prompt.`,
    ...
    year: "2025-Present",
    highlights: [
      "Monte Carlo sampler: n=5 independent inference passes per prompt at identical settings",
      ...
      "62-prompt benchmark dataset across 5 categories × 3 difficulty levels = 310 total inference calls",
      "Key finding: temperature has negligible effect on instability (variance &lt; 0.001 across T=0.1–0.9)",
      "Key finding: coding ranks last in reliability (instability 0.293) despite Qwen's marketed coding strength",
      "Key finding: 11.8% null generation rate — a silent production failure mode",
      "Live Next.js dashboard with real-time telemetry, escalation badges, and offline JSON reports",
      "Published benchmark dataset and research analysis notebook on Kaggle",
    ],
    techStack: [
    ...
    learnings: [
      ...
      "Temperature is not a reliability control knob for Qwen 2.5-7B — output variance is prompt-complexity-driven",
      "Silent failure modes (null generation) are more dangerous than noisy ones in production systems",
